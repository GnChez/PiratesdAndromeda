...
class RegisterFragment : Fragment() {
                    ...
                    viewModel.authState.observe(viewLifecycleOwner) { state ->
                        when (state) {
                            ...
                            AuthState.SUCCESS -> {
                                ...
                                Snackbar.make(binding.root, getString(R.string.auth_register_success), Snackbar.LENGTH_SHORT).show()
    ...
    private fun setupListeners() {
        ...
        binding.btnRegistrate.setOnClickListener {
            ...
            when {
                email.isEmpty() -> {
                    Snackbar.make(binding.root, getString(R.string.auth_empty_email), Snackbar.LENGTH_SHORT).show()
                }
                username.isEmpty() -> {
                    Snackbar.make(binding.root, getString(R.string.auth_empty_username), Snackbar.LENGTH_SHORT).show()
                }
                password.isEmpty() -> {
                    Snackbar.make(binding.root, getString(R.string.auth_empty_password), Snackbar.LENGTH_SHORT).show()
                }
                password.length < 6 -> {
                    Snackbar.make(binding.root, getString(R.string.auth_password_too_short), Snackbar.LENGTH_SHORT).show()
                }
                password != passwordRep -> {
                    Snackbar.make(binding.root, getString(R.string.auth_password_mismatch), Snackbar.LENGTH_SHORT).show()
                }
                else -> {
